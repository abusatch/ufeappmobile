...
if(!empty($_GET['jenis'])){
    $ew = mysqli_query($koneksi,"select * from tb_menu where jenis = '$_GET[jenis]' order by sort");
}else{
    $ew = mysqli_query($koneksi,"select * from tb_menu where jenis = 'DEMARCHES' order by sort");
}
